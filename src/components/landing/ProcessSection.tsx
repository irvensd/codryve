...
import { easeLux, transitionItem, viewportOnce } from './motion';
  ...
  return (
    <Section id="process" className="border-t border-zinc-200/60 bg-[#f4f4f2] py-24 sm:py-28 lg:py-32">
      ...
      <div className="relative mt-16 lg:mt-20">
        <div className="absolute left-0 right-0 top-[2.25rem] hidden h-px bg-gradient-to-r from-transparent via-zinc-300 to-transparent lg:block" aria-hidden />
        ...
        <div className="grid gap-10 md:grid-cols-2 lg:grid-cols-4 lg:gap-6">
          {steps.map((step, index) => (
            <motion.div
              ...
              transition={{ ...transitionItem(index), duration: reduce ? 0 : 0.5, ease: easeLux }}
              className="relative rounded-3xl border border-zinc-200/70 bg-white p-7 text-left shadow-[0_1px_0_0_rgba(255,255,255,0.8)_inset] sm:p-8 lg:text-center"
            >
              <div className="flex items-center gap-4 lg:flex-col lg:gap-0">
                <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full border border-zinc-200 bg-zinc-50 text-xs font-semibold tabular-nums text-zinc-900 lg:mx-auto">
                  {String(index + 1).padStart(2, '0')}
                </div>
                <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl border border-zinc-200/80 bg-white text-zinc-800 lg:mx-auto lg:mt-5">
